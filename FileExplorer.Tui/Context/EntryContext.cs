...
public class EntryContext : IEntryContext
{
    ...
    public required TuiListBox<TuiLabel> Menu { get; set; }
    public InputListener? Listener { get; set; }
    public object EntryLock { get; set; } = new();
    ...
    public List<TuiListBoxItem<TuiLabel>> SelectedItems { get; } = new();
    ...
    public required ManualResetEventSlim ExitEvent { get; set; }
    public required bool ForceTtyInput { get; set; }
    ...
    public static IEntryContext Create(MenuContextOptions options)
    {
        ...
        context.Listener = options.ForceTtyInput ? new TtyInputListener() : InputListener.New();
        ...
        context.OnClickDir(new TuiLabel(Directory.GetCurrentDirectory()), false);
        ...
        context.Listener.OnKeyDown += options.HandleKeyDown;
        context.Listener.OnKeyUp += options.HandleKeyUp;
        context.Listener.OnKeyJustPressed += options.HandleKeyJustPressed;

        context.Menu.MenuUpdate += options.MenuUpdate;
    ...
    public void RefreshItems()
    {
        ...
        Task.Run(() =>
        {
            ...
            {
                TuiLabel item = new("..", Color.White)
                {
                    Prefix = "   ",
                };
                
                TuiListBoxItem<TuiLabel> lbItem = new(item)
                {
                    Data =
                    {
                        {"ItemType", "Folder"},
            ...
            Menu.AddItemRange(
                dirPaths
                    .Select(dirPath =>
                    {
                        TuiLabel item = new(Path.GetFileName(dirPath), Color.FromRgbString(Blue))
                        {
                        ...
                        lbItem.OnClick += () => OnClickDir(new TuiLabel(dirPath));
            ...
            Menu.AddItemRange(
                fileNames
                    .Select(filePath =>
                    {
                        TuiLabel item = new(Path.GetFileName(filePath))
                        {
                            Prefix = $"{Color.White.ToAnsi()}\x1b[1m🗏︎  \x1b[0m",
                        };

                        return new TuiListBoxItem<TuiLabel>(item)
                        {
                            Data =
                            {
                                {"ItemType", "File"},
    ...
    public void UpdateFolderAttributes(TuiListBoxItem<TuiLabel> dir, CancellationToken token)
    {
        if (token.IsCancellationRequested)
        {
            return;
    ...
    public void UpdateFileAttributesFast(TuiListBoxItem<TuiLabel> file, string? mime, CancellationToken token)
    {
        if (token.IsCancellationRequested)
        {
            return;
    ...
    public void UpdateFileAttributesAccurate(TuiListBoxItem<TuiLabel> file, string? mime)
    {
        if (!file.Data.TryGetValue("ItemType", out string? fileType) || fileType != "File")
        {
            return;
    ...
    public void OnClickDir(TuiLabel sender, bool saveToHistory = true)
    {
        ...
        {
            TuiListBoxItem<TuiLabel>? item = Menu.Items.FirstOrDefault(item => item.Item.Text == e.Name);
            if (item != null)
            {
        ...
        {
            TuiListBoxItem<TuiLabel>? item = Menu.Items.FirstOrDefault(item => item.Item.Text == e.OldName);
            if (item != null)
            {
    ...
    public string? Input(string inputText, string startValue = "", bool enterNull = false, bool escapeNo = false, bool inputHidden = false)
    {
        ...
        void Redraw()
        {
            ...
            int offsetCursor = cursor;
            int offsetTop = top;
            for (int i = 0; i < (cursor + Color.TrimAnsi(inputText).Length) / Console.WindowWidth; i++)
            {
                offsetCursor -= Console.WindowWidth;
                offsetTop++;
            }
            
            Console.SetCursorPosition(offsetCursor + Color.TrimAnsi(inputText).Length, offsetTop);
            Console.CursorVisible = true;
        }
        ...
        void OnKeyUp(Key key, KeyUpEventArgs e)
        {
            if (key == Key.Escape)
            {
    ...
    {
        TuiLabel? item = Menu.SelectedItem.Item;
