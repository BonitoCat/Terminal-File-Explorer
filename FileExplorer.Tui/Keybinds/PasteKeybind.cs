...
public class PasteKeybind(IEntryContext context) : Keybind(context)
{
            ...
            else if (mode == ClipboardMode.Cut)
            {
                foreach (string itemPath in paths)
                {
                    try
                    {
                        ...
                        TuiListBoxItem<TuiLabel>? item = _context.Menu.Items.FirstOrDefault(item => item.Item.Text == name);
                        if (item != null)
                        {
